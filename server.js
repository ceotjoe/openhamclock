...
function tryDXSpiderNode(node, userCallsign = null) {
  return new Promise((resolve) => {
    ...
    const loginCallsign = userCallsign
      ? `${userCallsign.toUpperCase()}${DXSPIDER_SSID}`
      : "GUEST";
...
app.get("/api/dxcluster/paths", async (req, res) => {
  ...
  const cacheKey =
    source === "custom" ? `custom-${customHost}-${customPort}` : "default";

  // Check cache first (but not for custom sources - they might have different data)
  if (
    source !== "custom" &&
    Date.now() - dxSpotPathsCache.timestamp < DXPATHS_CACHE_TTL &&
    dxSpotPathsCache.paths.length > 0
  ) {
    logDebug(
      "[DX Paths] Returning",
      dxSpotPathsCache.paths.length,
      "cached paths",
    );
  ...
  try {
    ...
    if (source === "custom" && customHost) {
      logDebug(
        `[DX Paths] Trying custom telnet: ${customHost}:${customPort} as ${userCallsign || "GUEST"}`,
      );
    ...
    if (newSpots.length === 0) {
      ...
      const validPaths = dxSpotPathsCache.allPaths.filter(
    ...
    const existingValidPaths = dxSpotPathsCache.allPaths.filter(
      (p) => now - p.timestamp < DXPATHS_RETENTION,
    );
...
app.get("/api/callsign/:call", async (req, res) => {
  ...
  try {
    ...
    const response = await fetch(
      `https://www.hamqth.com/dxcc.php?callsign=${callsign}`,
    );
    if (response.ok) {
      const text = await response.text();
...
app.get("/api/satellites/tle", async (req, res) => {
  try {
    ...
    tleResponses.forEach((text, index) => {
      ...
      for (let i = 0; i < lines.length - 2; i += 3) {
        ...
        if (name && line1 && line1.startsWith("1 ")) {
          ...
          if (hamSat) {
            // Priority 1: Built-in HAM_SATELLITES metadata
            tleData[key] = { ...hamSat, tle1: line1, tle2: line2 };
          } else {
            // Priority 2: Standard TLE data
            tleData[key] = {
              norad: noradId,
              name: name,
              color: "#cccccc",
          ...
          // Merge Frequency Data if available (enriching both cases)
          if (freqData) {
            tleData[key].uplink = freqData.uplink; // e.g. "145.850-145.950"
            tleData[key].downlink = freqData.downlink; // e.g. "435.100"
            tleData[key].beacon = freqData.beacon;
            tleData[key].mode = freqData.mode || tleData[key].mode; // Prefer Palewire mode description
            tleData[key].callsign = freqData.callsign;
          }
        }
      }
    });
    ...
    // Update cache only if we found satellites
    const count = Object.keys(tleData).length;
    if (count > 0) {
      logInfo(`[Satellites] Cached ${count} satellites (with ${freqMap.size} frequency records)`);
      tleCache = { data: tleData, timestamp: Date.now() };
    } else {
      logWarn('[Satellites] No satellites parsed from TLE source');
    }

    return res.json(tleData);
  } catch (error) {
    console.error("[Satellites] Critical error in TLE update:", error);
...
function parseDecodeMessage(text) {
  ...
  const cqMatch = text.match(
    /^CQ\s+(?:(\S+)\s+)?([A-Z0-9/]+)\s+([A-R]{2}\d{2}[a-x]{0,2})?/i,
  );
  if (cqMatch) {
    result.type = "CQ";
    result.modifier =
      cqMatch[1] && !cqMatch[1].match(/^[A-Z0-9/]{3,}$/) ? cqMatch[1] : null;
